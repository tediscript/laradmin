                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-4">
                        <form method="GET" action="{{ route('admin.users.index') }}" class="flex gap-3 items-center">
                            ...
                            @if($search)
                                <a href="{{ route('admin.users.index', $sortParams) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-md font-semibold text-xs uppercase tracking-widest hover:bg-gray-50 transition duration-150">
                                    Clear
                                </a>
                            @endif
                        </form>
                                ...
                                <thead>
                                    <tr class="border-b border-gray-200">
                                        ...
                                        @foreach($columns as $column => $label)
                                            ...
                                            @endphp
                                            <th class="pb-3 font-medium text-sm text-gray-700 {{ $column === 'created_at' ? '' : '' }}">
                                                <a href="{{ route('admin.users.index', $linkParams) }}" class="inline-flex items-center gap-1 hover:text-gray-900 {{ $isActive ? 'text-gray-900' : '' }}">
                                                    {{ $label }}
                                                    @if($isActive)
                                                        <span class="text-xs">
                    ...
                    @else
                        <p class="text-gray-500 text-sm">
                            @if($search)
                                No users found matching "{{ $search }}". <a href="{{ route('admin.users.index', $sortParams) }}" class="text-gray-700 underline">Clear search.</a>
                            @else
                                No users found. <a href="{{ route('admin.users.create') }}" class="text-gray-700 underline">Create a new user.</a>
                            @endif
                        </p>
                    @endif
                </div>

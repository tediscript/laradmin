<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.permissions.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white rounded-md font-semibold text-xs uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 transition duration-150">
                Add Permission
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    {{-- Search Input --}}
                    <div class="mb-4">
                        <form method="GET" action="{{ route('admin.permissions.index') }}" class="flex gap-3 items-center">
                            ...
                            @if($search)
                                <a href="{{ route('admin.permissions.index', $sortParams) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-md font-semibold text-xs uppercase tracking-widest hover:bg-gray-50 transition duration-150">
                                    Clear
                                </a>
                            @endif
                        </form>
                                    ...
                                    <tr class="border-b border-gray-200">
                                        ...
                                        @foreach($columns as $column => $label)
                                            ...
                                            <th class="pb-3 font-medium text-sm text-gray-700">
                                                <a href="{{ route('admin.permissions.index', $linkParams) }}" class="inline-flex items-center gap-1 hover:text-gray-900 {{ $isActive ? 'text-gray-900' : '' }}">
                                                    {{ $label }}
                                                    @if($isActive)
                                                        <span class="text-xs">
                                        ...
                                        <tr class="border-b border-gray-100 last:border-0">
                                            ...
                                            <td class="py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    <a href="{{ route('admin.permissions.edit', $permission) }}" class="inline-flex items-center px-3 py-1.5 bg-white border border-gray-300 text-gray-700 shadow-sm hover:bg-gray-50 rounded-md font-semibold text-xs uppercase tracking-widest focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 transition duration-150">
                                                        Edit
                                                    </a>
                    ...
                    @else
                        <p class="text-gray-500 text-sm">
                            @if($search)
                                No permissions found matching "{{ $search }}". <a href="{{ route('admin.permissions.index', $sortParams) }}" class="text-gray-700 underline">Clear search.</a>
                            @else
                                No permissions found. <a href="{{ route('admin.permissions.create') }}" class="text-gray-700 underline">Create your first permission.</a>
                            @endif
                        </p>
                    @endif
                </div>
